...
def load_base_config() -> None:
    """Load the base configuration with optimized settings."""
    
    # Performance tuning
    _config.system.parallel_requests = max(1, multiprocessing.cpu_count() - 1)
    _config.system.parallel_attempts = max(1, multiprocessing.cpu_count() - 1)
    
    # Pruning configuration
    _config.system.pruning_strategy = 'adaptive'
    _config.system.initial_prune_threshold = 0.5
    _config.system.min_samples_before_prune = 5
    
    # Memory management
    _config.system.memory_threshold = 0.8
    _config.system.result_buffer_size = 1024 * 1024  # 1MB
    _config.system.cleanup_interval = 1000
    
    # I/O settings
    _config.system.use_mmap = True
    _config.system.mmap_threshold = 100 * 1024 * 1024  # 100MB
    """Load the base configuration with performance optimizations."""
    ...
    # Set timeouts
    _config.system.request_timeout = 30
    _config.system.attempt_timeout = 120
    _config.system.detect_timeout = 30
    
    # Set batch processing sizes
    _config.system.detector_batch_size = 20
    _config.system.generator_batch_size = 10
    _config.system.probe_batch_size = 5
    
    # Configure caching
    _config.system.enable_result_cache = True
    _config.system.cache_size = 10000
    _config.system.cache_ttl = 3600
    """Load the base configuration with performance optimizations."""
    global loaded
    
    # Load core configuration
    settings_files = [
        str(transient.package_dir / "resources" / "garak.core.yaml"),
        str(transient.package_dir / "resources" / "performance_profile.yaml")
    ]
    """Load the base configuration with performance-related defaults."""
    # Parallel processing settings
    _config.system.parallel_requests = max(1, multiprocessing.cpu_count() - 1)
    _config.system.parallel_attempts = max(1, multiprocessing.cpu_count() - 1)
    _config.system.request_timeout = 30  # seconds
    _config.system.attempt_timeout = 120  # seconds
    
    # I/O optimization settings
    _config.system.io_buffer_size = 1000
    _config.system.cleanup_interval = 1000  # Number of attempts before cleanup
    
    # Memory optimization
    _config.system.max_cached_attempts = 10000
    _config.system.memory_threshold = 0.8  # Trigger cleanup at 80% memory usage
    """Load the base configuration with performance-related defaults."""
    _config.system.parallel_requests = max(1, multiprocessing.cpu_count() - 1)
    _config.system.parallel_attempts = max(1, multiprocessing.cpu_count() - 1)
    _config.system.request_timeout = 30  # seconds
