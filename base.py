...
class Probe(Configurable):
    ...
    def _execute_all(self, attempts) -> Iterable[garak.attempt.Attempt]:
        """Handles sending a set of attempts to the generator with optimized batching."""
        chunk_size = getattr(_config.system, 'probe_chunk_size', 5)
        if len(attempts) > chunk_size * 2:  # Only batch if we have enough attempts
            chunked_attempts = [attempts[i:i + chunk_size] 
                              for i in range(0, len(attempts), chunk_size)]
            all_results = []
            for chunk in chunked_attempts:
                all_results.extend(self._execute_all(chunk))
            return all_results
        ...
        if (
            ...
            from garak.parallel_utils import process_in_parallel
            
            chunk_size = min(5, max(1, len(attempts) // (_config.system.parallel_attempts * 2)))
            results = process_in_parallel(
                self._execute_attempt,
                attempts,
                num_processes=_config.system.parallel_attempts,
                chunk_size=chunk_size,
                timeout=getattr(_config.system, 'attempt_timeout', None)
            )
    ...
    def probe(self, generator):
        ...
        while len(nodes_to_explore):
            ...
            for attempt in attempts_completed:
                ...
                attempt.detector_results[self.primary_detector] = detector_manager.detect_batch(
                    [attempt], self.primary_detector
                )[0]
